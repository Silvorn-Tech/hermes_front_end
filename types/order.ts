 ...
 * `order` field of a create/cancel/close response). `botId` has no backend
 * equivalent for real orders (optional, mock-only). `price` is absent for
 * MARKET orders. `executedQuantity`/`averageFillPrice` are only meaningful
 * once Binance has reported a fill — null until then, never assumed to
